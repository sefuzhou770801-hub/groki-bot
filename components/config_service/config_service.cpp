...
// Operator-set name override seeded by start() from DeviceConfig.device_name.
// Empty → fall back to autogenerated Stackchan-XXXXXX in on_sync(). Plain
// static here (rather than reading from gatt's g_active) keeps the host task
// path independent of GATT internals.
static char g_configured_name[32] = {};
...
static void on_sync()
{
    ...
    // Honor the operator-set override when present; otherwise build the name
    // from the *Wi-Fi STA* MAC lower 3 bytes — the same bytes the mDNS
    // hostname uses (wifi_config_service::compose_hostname), so a central can
    // derive the .local hostname from the BLE name. STA is the ESP32 base MAC
    // (BT = base + 2), so it also matches what the router shows.
...
{
    // Snapshot the operator-set name into our static so on_sync() (which fires
    // later from the NimBLE host task) can use it. Truncates above 24 bytes.
    if (!current.device_name.empty()) {
        std::snprintf(g_configured_name, sizeof(g_configured_name), "%s", current.device_name.c_str());
    } else {
        g_configured_name[0] = '\0';
    }
