...
## 4. atoms3r / atoms3 の storage パーティション溢れ（原因は解消、未検証）

**経緯**: 以前の版は顔アニメーション資源（約 5 MB）を storage パーティションの
SPIFFS イメージとして焼いており、storage が 1 MiB の `atoms3r` / `atoms3` では
`spiffsgen.py` が `SpiffsFullError` で止まり、両ボードのファームを出せなかった。

**現状**: v0.2.0 でこの資源と描画コンポーネントを削除したため、原因はなくなった。
ただし両ボードのビルドと実機確認はまだ行っていない。CI の `release.yml` では
両ボードを `allow_failure` のまま残し、通った場合だけ Release に添付する。
