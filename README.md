...
3. In `gateway/.env`, set `STACKCHAN_TOOL_BOT` to the agent's name (for example `STACKCHAN_TOOL_BOT=助手`). The feature stays off until this is set.
...
5. Say "Hi Grok, look up tomorrow's weather". The robot answers "已经发给助手啦" (sent to the agent) and then reads the agent's replies.
