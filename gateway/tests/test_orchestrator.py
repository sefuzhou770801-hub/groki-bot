...
class _StreamingPCMEngine(TTSEngine):
    """Engine that yields PCM in chunks and observes when frames ship."""

    def __init__(self, esp32, name: str = "qwen3") -> None:
        self.name = name
        self._esp32 = esp32
        self.frames_sent_after_first_chunk: int | None = None

    async def synthesize(self, text: str, **opts: Any):
        yield b"\x01\x00" * 960
        self.frames_sent_after_first_chunk = len(self._esp32.frames)
        yield b"\x02\x00" * 960
...
@pytest.mark.asyncio
async def test_pipeline_streams_first_chunk_before_engine_finishes(fake_encode):
    """A streaming engine's first PCM chunk reaches the device immediately."""
    esp32 = _FakeESP32(connected=True)
    gateway = _FakeGateway(esp32)
    engine = _StreamingPCMEngine(esp32)

    reg = EngineRegistry()
    reg.register(engine)

    result = await synthesize_and_send(
        {"text": "こんにちは", "voice": "qwen3"},
        gateway=gateway,
        registry=reg,
    )

    assert engine.frames_sent_after_first_chunk == 1
    assert result["frame_count"] == 2
    assert esp32.frames == [b"opus_frame_0", b"opus_frame_0"]
