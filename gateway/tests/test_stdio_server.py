...
async def test_list_tools_includes_get_head_angles():
    ...
    assert "set_voice_mode" in tool_names
...
@pytest.mark.asyncio
async def test_get_head_angles_uses_usb_when_ws_is_down(monkeypatch):
    """USB transport is a valid device path for ordinary MCP tools."""
    calls = []
    ...
    assert calls == [("self.robot.get_head_angles", {}, 2.0)]
    assert json.loads(result.root.content[0].text) == {"yaw": 4, "pitch": 8}


@pytest.mark.asyncio
async def test_get_status_reports_usb_connected_when_ws_is_down(monkeypatch):
    """get_status should reflect the actual usable USB control path."""
    ...
    status = json.loads(result.root.content[0].text)
