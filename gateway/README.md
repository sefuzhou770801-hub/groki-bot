...
"research X for me": Gemini answers "已经发给总管啦" (sent to the agent)
...
1. Pick the agent name from `gbot bots list` (or create one, for example
   `gbot bots create --name 总管`) and add it to `.env`:
   `STACKCHAN_TOOL_BOT=总管`. This is what turns the feature on.
...
curl -s -X POST http://127.0.0.1:18770/send \
  ...
  -d '{"text": "Reply with one short sentence: hello", "target": "总管"}'
```
