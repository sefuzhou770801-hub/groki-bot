...
| **2. 加网关** | 「Hi Grok」唤醒词，语音经电脑走 Gemini Live；Claude 等 MCP 客户端能让机器人说话、读取它的状态 | 用法 1 的全部，加一台和机器人在同一 Wi-Fi、运行 [gateway/](gateway/README.zh-CN.md) 的电脑，以及一把 Gemini API 密钥 |
...
3. 在蓝牙设置页的「对话」标签页：
   ...
   - 点「保存并重启」。
4. 在电脑上打开 <http://127.0.0.1:8766/debug/status>，看到 `"device": {"connected": true}` 后说「Hi Grok」。
