...
|---|---|---|
| `robot-demo` | Gives a real task to the `grok` CLI; the robot shows a thinking face while it works, then an excited face and a speech balloon with the result | zsh, curl, the `grok` CLI on `PATH` |
...
```sh
ROBOT_HOST=<robot LAN address> ROBOT_TOKEN=<MCP token> \
  ./tools/face-demo/robot-demo "count the TODOs in this repository"
```

`ROBOT_CWD` sets the task's working directory (default: current directory). The script exits with an error when `ROBOT_HOST`, `ROBOT_TOKEN` or `grok` is missing.
...
- `robot-demo`：把真实任务交给 `grok` 命令行，干活时机器人显示思考脸，完成后显示兴奋脸，并用气泡显示结果摘要。需要设置 `ROBOT_HOST`、`ROBOT_TOKEN`，`grok` 要在 `PATH` 里。
